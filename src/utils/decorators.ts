...
import { AsyncMethod, SyncMethod, WyvernProtocolError } from '../types';
...
const contractCallErrorTransformer = (error: Error) => {
    if (_.includes(error.message, constants.INVALID_JUMP_PATTERN)) {
        return new Error(WyvernProtocolError.InvalidJump);
    }
    if (_.includes(error.message, constants.OUT_OF_GAS_PATTERN)) {
        return new Error(WyvernProtocolError.OutOfGas);
    }
    return error;
};
