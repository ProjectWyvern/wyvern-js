...
class WyvernTokenContract extends base_contract_1.BaseContract {
    constructor(address, supportedProvider, txDefaults, logDecodeDependencies, deployedBytecode = WyvernTokenContract.deployedBytecode, encoderOverrides) {
        super('WyvernToken', WyvernTokenContract.ABI(), address, supportedProvider, txDefaults, logDecodeDependencies, deployedBytecode, encoderOverrides);
        this._methodABIIndex = {};
        utils_1.classUtils.bindAll(this, ['_abiEncoderByFunctionSignature', 'address', '_web3Wrapper']);
        this._subscriptionManager = new base_contract_1.SubscriptionManager(WyvernTokenContract.ABI(), this._web3Wrapper);
        WyvernTokenContract.ABI().forEach((item, index) => {
    ...
    static deployFrom0xArtifactAsync(artifact, supportedProvider, txDefaults, logDecodeDependencies, merkleRoot, totalUtxoAmount) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return WyvernTokenContract.deployAsync(bytecode, abi, provider, txDefaults, logDecodeDependenciesAbiOnly, merkleRoot, totalUtxoAmount);
        });
    }
    static deployWithLibrariesFrom0xArtifactAsync(artifact, libraryArtifacts, supportedProvider, txDefaults, logDecodeDependencies, merkleRoot, totalUtxoAmount) {
        return __awaiter(this, void 0, void 0, function* () {
            assert_1.assert.doesConformToSchema('txDefaults', txDefaults, json_schemas_1.schemas.txDataSchema);
            if (artifact.compilerOutput === undefined) {
                throw new Error('Compiler output not found in the artifact file');
            ...
            const libraryAddresses = yield WyvernTokenContract._deployLibrariesAsync(artifact, libraryArtifacts, new web3_wrapper_1.Web3Wrapper(provider), txDefaults);
            const bytecode = (0, base_contract_1.linkLibrariesInBytecode)(artifact, libraryAddresses);
            return WyvernTokenContract.deployAsync(bytecode, abi, provider, txDefaults, logDecodeDependenciesAbiOnly, merkleRoot, totalUtxoAmount);
        });
    }
    static deployAsync(bytecode, abi, supportedProvider, txDefaults, logDecodeDependencies, merkleRoot, totalUtxoAmount) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            [merkleRoot,
                totalUtxoAmount
            ] = base_contract_1.BaseContract._formatABIDataItemList(constructorAbi.inputs, [merkleRoot,
                totalUtxoAmount
            ], base_contract_1.BaseContract._bigNumberToString);
            const iface = new ethers.utils.Interface(abi);
            const deployInfo = iface.deployFunction;
            const txData = deployInfo.encode(bytecode, [merkleRoot,
                totalUtxoAmount
            ]);
            ...
            utils_1.logUtils.log(`WyvernToken successfully deployed at ${txReceipt.contractAddress}`);
            const contractInstance = new WyvernTokenContract(txReceipt.contractAddress, provider, txDefaults, logDecodeDependencies);
            ...
            for (const link of Object.values(links)) {
                for (const libraryName of Object.keys(link)) {
                    if (!libraryAddresses[libraryName]) {
                        ...
                        yield WyvernTokenContract._deployLibrariesAsync(libraryArtifact, libraryArtifacts, web3Wrapper, txDefaults, libraryAddresses);
                        // Deploy this library.
    ...
    getFunctionSignature(methodName) {
        ...
        const methodAbi = WyvernTokenContract.ABI()[index]; // tslint:disable-line:no-unnecessary-type-assertion
    ...
    MULTIPLIER() {
        const self = this;
        const functionSignature = 'MULTIPLIER()';
    ...
    maximumRedeemable() {
        const self = this;
        const functionSignature = 'maximumRedeemable()';
        const selector = self._lookupAbiEncoder(functionSignature).getSelector();
        return {
            selector,
            callAsync(callData = {}, defaultBlock) {
    ...
    pubKeyToEthereumAddress(pubKey) {
        ...
        const functionSignature = 'pubKeyToEthereumAddress(bytes)';
        const selector = self._lookupAbiEncoder(functionSignature).getSelector();
        return {
            selector,
            callAsync(callData = {}, defaultBlock) {
    ...
    MINT_AMOUNT() {
        const self = this;
        const functionSignature = 'MINT_AMOUNT()';
        const selector = self._lookupAbiEncoder(functionSignature).getSelector();
        return {
            selector,
            callAsync(callData = {}, defaultBlock) {
    ...
    redeemUTXO(txid, outputIndex, satoshis, proof, pubKey, isCompressed, v, r, s) {
        const self = this;
        ...
        const functionSignature = 'redeemUTXO(bytes32,uint8,uint256,bytes32[],bytes,bool,uint8,bytes32,bytes32)';
        const selector = self._lookupAbiEncoder(functionSignature).getSelector();
        return {
            selector,
            sendTransactionAsync(txData, opts = { shouldValidate: true }) {
    ...
    canRedeemUTXOHash(merkleLeafHash, proof) {
        const self = this;
        assert_1.assert.isString('merkleLeafHash', merkleLeafHash);
        assert_1.assert.isArray('proof', proof);
        const functionSignature = 'canRedeemUTXOHash(bytes32,bytes32[])';
        const selector = self._lookupAbiEncoder(functionSignature).getSelector();
        return {
            selector,
            callAsync(callData = {}, defaultBlock) {
    ...
    releaseTokens(destination) {
        const self = this;
        assert_1.assert.isString('destination', destination);
        const functionSignature = 'releaseTokens(address)';
        const selector = self._lookupAbiEncoder(functionSignature).getSelector();
        return {
            selector,
            sendTransactionAsync(txData, opts = { shouldValidate: true }) {
    ...
    validateSignature(hash, v, r, s, expected) {
        const self = this;
        ...
        const functionSignature = 'validateSignature(bytes32,uint8,bytes32,bytes32,address)';
        const selector = self._lookupAbiEncoder(functionSignature).getSelector();
        return {
            selector,
            callAsync(callData = {}, defaultBlock) {
    ...
    verifyProof(proof, merkleLeafHash) {
        ...
        assert_1.assert.isArray('proof', proof);
        assert_1.assert.isString('merkleLeafHash', merkleLeafHash);
        const functionSignature = 'verifyProof(bytes32[],bytes32)';
        const selector = self._lookupAbiEncoder(functionSignature).getSelector();
        return {
            selector,
            callAsync(callData = {}, defaultBlock) {
    ...
    canRedeemUTXO(txid, originalAddress, outputIndex, satoshis, proof) {
        const self = this;
        assert_1.assert.isString('txid', txid);
        ...
        assert_1.assert.isBigNumber('satoshis', satoshis);
        assert_1.assert.isArray('proof', proof);
        const functionSignature = 'canRedeemUTXO(bytes32,bytes20,uint8,uint256,bytes32[])';
        const selector = self._lookupAbiEncoder(functionSignature).getSelector();
        return {
            selector,
            callAsync(callData = {}, defaultBlock) {
    ...
    ecdsaVerify(addr, pubKey, v, r, s) {
        ...
        assert_1.assert.isString('r', r);
        assert_1.assert.isString('s', s);
        const functionSignature = 'ecdsaVerify(address,bytes,uint8,bytes32,bytes32)';
        const selector = self._lookupAbiEncoder(functionSignature).getSelector();
        return {
            selector,
            callAsync(callData = {}, defaultBlock) {
    ...
    pubKeyToBitcoinAddress(pubKey, isCompressed) {
        const self = this;
        assert_1.assert.isString('pubKey', pubKey);
        assert_1.assert.isBoolean('isCompressed', isCompressed);
        const functionSignature = 'pubKeyToBitcoinAddress(bytes,bool)';
        const selector = self._lookupAbiEncoder(functionSignature).getSelector();
        return {
            selector,
            callAsync(callData = {}, defaultBlock) {
    ...
    rootUTXOMerkleTreeHash() {
        const self = this;
        const functionSignature = 'rootUTXOMerkleTreeHash()';
        const selector = self._lookupAbiEncoder(functionSignature).getSelector();
        return {
            selector,
            callAsync(callData = {}, defaultBlock) {
    ...
    totalRedeemed() {
        const self = this;
        const functionSignature = 'totalRedeemed()';
        const selector = self._lookupAbiEncoder(functionSignature).getSelector();
        return {
            selector,
            callAsync(callData = {}, defaultBlock) {
    ...
    SATS_TO_TOKENS() {
        const self = this;
        const functionSignature = 'SATS_TO_TOKENS()';
        const selector = self._lookupAbiEncoder(functionSignature).getSelector();
        return {
            selector,
            callAsync(callData = {}, defaultBlock) {
    ...
    subscribe(eventName, indexFilterValues, callback, isVerbose = false, blockPollingIntervalMs) {
        ...
        const subscriptionToken = this._subscriptionManager.subscribe(this.address, eventName, indexFilterValues, WyvernTokenContract.ABI(), callback, isVerbose, blockPollingIntervalMs);
    ...
    getLogsAsync(eventName, blockRange, indexFilterValues) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            assert_1.assert.doesConformToSchema('indexFilterValues', indexFilterValues, json_schemas_1.schemas.indexFilterValuesSchema);
            const logs = yield this._subscriptionManager.getLogsAsync(this.address, eventName, blockRange, indexFilterValues, WyvernTokenContract.ABI());
            return logs;
        });
    }
}
